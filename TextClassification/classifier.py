...
class Classifier(object):
    ...
    # k-folds 交叉验证
    def cross_validate(self, model, dataset, folds=10):
        test_acc_lst = []

        for i, (train_data, dev_data, test_data) in enumerate(cv_data_split(dataset, folds)):
            print('='*10, i, '='*10)
            # self._classifier = model  # 赋值，传对象的引用
            # self._classifier = copy.copy(model)  # 浅拷贝, 只拷贝父对象, 不会拷贝对象的内部的子对象
            self._classifier = copy.deepcopy(model)  # 深拷贝, 拷贝父对象及子对象
    ...
    def evaluate(self, test_data):
        self._classifier.eval()
        test_acc = 0
        for test_batch_data in get_batch(test_data, self._args.batch_size):
